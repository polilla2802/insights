<script lang="ts">
	...
	const exp = $derived(experiencias.find((e) => e.id === page.params.id));
	...
	const total = $derived((exp?.precio ?? 0) * personas);
</script>

<svelte:head>
	<title>{exp ? `${exp.titulo} — ConoCé-DMX` : 'Experiencia — ConoCé-DMX'}</title>
</svelte:head>
	...
	<div class="max-w-4xl mx-auto px-4 md:px-6 py-6">
		<div class="md:grid md:grid-cols-3 md:gap-8">
			...
			<div class="md:col-span-2">
				...
				<p class="text-sm text-gray-500 mb-4">{exp.lugar} · {exp.alcaldia} · {formatDuracion(exp.duracion)}</p>
				...
				<div class="grid grid-cols-2 gap-3 mb-6">
					...
					<div class="bg-gray-50 rounded-xl p-4">
						...
						<p class="font-semibold text-gray-900">{exp.alcaldia}</p>
					</div>
				</div>
			</div>
...
{:else}
	<div class="flex flex-col items-center justify-center py-32 text-center">
		<p class="text-gray-500 font-medium">Experiencia no encontrada</p>
		<a href="/experiencias" class="text-magenta-600 text-sm mt-2 hover:underline">Ver todas las experiencias</a>
	</div>
{/if}
